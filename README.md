...
GitHubリポジトリは**Private**のままでOK(アカウント名がURLやコードから見えません)。

1. [Cloudflareダッシュボード](https://dash.cloudflare.com/) →「Workers & Pages」→「作成」→「Pages」→「Gitに接続」
2. GitHubアカウントを連携し、このリポジトリを選択
3. ビルド設定を入力して「保存してデプロイ」
   - **ビルドコマンド**: `npm run build`
   - **ビルド出力ディレクトリ**: `dist`
4. 数分待つと `https://プロジェクト名.pages.dev/` で公開されます
   - 以後、mainブランチに変更をpushするたびに自動で更新されます
   - 独自ドメインを使う場合は、プロジェクトの「カスタムドメイン」から追加できます
